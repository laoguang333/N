...
- 阅读位置按规范化 LF 文本中的 UTF-16 段落锚点保存；旧进度仍可按兼容路径恢复。
- 书内搜索支持大量命中结果的虚拟列表，不会把全部结果同时插入 DOM。
...
浏览器回归测试全部使用 `page.route()` 的隔离 mock API，不连接或写入本机真实书库：
...
其中包含扫描/书架竞态、阅读恢复、离线离开、重复关闭事件和大搜索结果集测试。旧页面首次升级后，建议刷新一次以使用新的进度同步缓存格式；未确认上传的最后位置可能暂时只保存在本机，网络恢复后会继续同步。
